...
class Parser
{
	...
	function parseScriptPart(template : String, startBrace : String, endBrace : String) : String
	{
		...
		while(++i < template.length)
		{
			...
			if(!insideDoubleQuote && !insideSingleQuote)
			{
				if(char == startBrace) {
					...
					if (stack < 0)
						return throw new ParserError( 'Unbalanced braces for block: ', pos,  template.substr(0, 100) );
				} else if(char == '"') {
		...
		return throw new ParserError( 'Failed to find a closing delimiter for the script block: ', this.pos, template.substr(0, 100) );
	}
	...
	private function parseString(str : String, modifier : String -> ParseResult, throwAtEnd : Bool) : String
	{
		...
		while(++i < str.length)
		{
			...
			if(!insideDoubleQuote && !insideSingleQuote)
			{
				...
				{
					case doneIncludeCurrent:
						return str.substr(0, i + 1);

					case doneSkipCurrent:
